void check_net() {
    ...
    if ( (time_now - gop_connect.point_time[0] >= 10) and gop_connect.how[0] != "Wait" ) {
...
//-----------------------------------------------------------------------------------------
    ...
    for ( int i = 1; i <= 3; i++ ) {
        ...
        if ( (time_now - gop_connect.point_time[i] >= 10) and gop_connect.how[i] != "Wait" ) {
...
//-----------------------------------------------------------------------------------------
        ...
        if ( gop_connect.how[i] == "Connectted") {
            ...
            int that2 = atoi(gop_connect.information[i].substr(num2+1, num3-num2).c_str());
             ...
             //cout << that1*that2 << endl;



            if ( that1*that2 > 2*info_cpu.cores*info_cpu.f and
