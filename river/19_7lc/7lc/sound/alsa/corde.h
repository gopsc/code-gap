...
//将配置空间限制为最接近目标的速率。
        ...
        if ((err = snd_pcm_hw_params_set_rate_near(chandle, ct_params, &rrate, 0)) < 0) {
            //cout << "Rate %iHz not available for %s: %s\n" << rate << snd_strerror(err) << endl;
            exit(0);}
        if ((int)rrate != rate) {
            //cout << "Rate doesn't match (requested %iHz, get %iHz)\n" << rate << snd_strerror(err) << endl;
            }
//--------------------------------------------------------------------------
...
//--------------------------------------------------------------------------
      __set_it:
        if ((err = snd_pcm_hw_params_set_buffer_size_near(chandle, c_params, &periodsize)) < 0) {
            //cout << "Unable to set buffer size %li for %s: %s\n" << *bufsize * 2 << snd_strerror(err) << endl;
            exit(0);}
//--------------------------------------------------------------------------
        if (period_size > 0)
...
//--------------------------------------------------------------------------
        if ((err = snd_pcm_hw_params_set_period_size_near(chandle, c_params, &periodsize, 0)) < 0) {
            ///cout << "Unable to set period size %li for %s: %s\n" << periodsize << snd_strerror(err) << endl;
            exit(0);}
//--------------------------------------------------------------------------
...
//--------------------------------------------------------------------------
            if (num_sound < 1000) {
                for (int i=0; i<21; i=i+1) {
                    //result_sound[num_sound*3 + i] = *((int*)buffer_sound+i);
                    //cout <<  *((int*)buffer_sound+i) << endl;

                    if ( *((int*)buffer_sound+i) > 10000000 ) {
                        num_aloud++;}
                    if ( num_count > 1 ) {
                        that_num =  to_string(*((int*)buffer_sound+i));
                        that_num += '\n';
                        file_append( buffer_path_sound,that_num );}}
                num_sound++;
//--------------------------------------------------------------------------
                if (that != *((int*)buffer_sound)) {
                    that  = *((int*)buffer_sound);
                    num_reset = 0;}
                else {
                    num_reset++;
                    if (num_reset >= 1000) {
                        //cout << "reset  :  " << num_sound << endl;
                        break;}}}
//--------------------------------------------------------------------------
            else {
                if ( num_aloud > 100 ) {
                    //cout << num_aloud << endl;
                    num_count = 3;
                    if ( buffer_path_sound == "" ) {
                        buffer_path_sound += path_sound;
                        buffer_path_sound += "/";
                        buffer_path_sound += info_time;
