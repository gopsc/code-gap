void socket_client::start(){
    ...
    file_append("/opt/gop/note/client.note", "Client start");
    file_append("/opt/gop/note/client.note", "\n");
    ...
    while (flag_client) {
        for (int i = 1 ; i <= point_ip[1]; i++) {
            if ( this->address_ip == info_ip[1][i][2] ) {
                goto leave;}}
        if (this->address_ip == gop_server.address_ip ) {
        ...
        if (connect(this->fd, (struct sockaddr *)&address, sizeof(address)) < 0) {}
            ...
            if ( this->how == "Wait" ) {
                ...
                file_append("/opt/gop/note/client.note", to_string(info_time));
                file_append("/opt/gop/note/client.note", "/n");
                file_append("/opt/gop/note/client.note", "Connectting to ");
                file_append("/opt/gop/note/client.note", this->address_ip);
                file_append("/opt/gop/note/client.note", "\n");;
            ...
            close(this->fd);
            this->fd = socket(AF_INET, SOCK_STREAM, 0);}
        leave:sleep(step_connect);}
    file_append("/opt/gop/note/client.note", to_string(info_time));
    file_append("/opt/gop/note/client.note", "/n");
    file_append("/opt/gop/note/client.note", "Client close with");
    file_append("/opt/gop/note/client.note", this->address_ip);
    file_append("/opt/gop/note/client.note", "\n");}
