void socket_server::start(void) {
    ...
    file_append("/opt/gop/note/server.note", "/n");
...
//===================================================================
    while(flag_server) {
        ...
        if(connection  < 0)  {
            ...
            if ( this->address_ip != that and  this->address_ip != box_blank ) {
                ...
                char that_ip[20];
                for ( ptr=0; ptr<that.length(); ptr++ ) {
                    that_ip[ptr] = that[ptr];}
                that_ip[ptr] = '\0';

                this->address_ip  = that_ip;
                this->information = "";
