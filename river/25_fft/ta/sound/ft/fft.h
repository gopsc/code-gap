...
void fft(int site) {
   ...
   //     - result_sound[site][(int)n] * sin(2*M_PI*w/N*(n-round)) * sin(2*M_PI*(double)w/N*round)
    ...
    for ( double w=1; w<=N/2; w++ ) {
        ...
        += (buffer_fft[(int)round][(int)w%32][1] * cos(2*M_PI*w*round/N)
           +buffer_fft[(int)round][(int)w%32][2] * sin(2*M_PI*w*round/N));



            bin_fft[(int)w][2]

        -= (buffer_fft[(int)round][(int)w%32][2] * cos(2*M_PI*w*round/N)
           -buffer_fft[(int)round][(int)w%32][1] * sin(2*M_PI*w*round/N));}
